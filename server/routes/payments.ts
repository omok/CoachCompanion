...
export function createPaymentsRouter(storage: IStorage): Router {
  ...
  router.post("/", requireTeamRolePermission(TEAM_PERMISSION_KEYS.MANAGE_PAYMENTS), async (req: Request<TeamParams>, res) => {
    try {
      ...
      // Log payment details including session information if applicable
      Logger.info(`Creating payment record: ${JSON.stringify({
        ...
        addPrepaidSessions: parsed.addPrepaidSessions,
      ...
      // Format the payment date for client
      const formattedPayment = formatPaymentDatesForClient([payment])[0];
